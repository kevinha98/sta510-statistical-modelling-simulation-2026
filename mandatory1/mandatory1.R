...
# STA510 mandatory assignment 1 - code for the [R] parts only, theory answers
# and discussion are in mandatory1_report.pdf

# just in case the default R library isn't writable on whatever machine
# this runs on
...
# only real dependency is MASS (fitdistr, problem 3d)
if (!requireNamespace("MASS", quietly = TRUE)) {
  message("installing MASS...")
  install.packages("MASS")
  if (!requireNamespace("MASS", quietly = TRUE)) {
    stop("could not install MASS - install it manually and re-run")
  }
}
library(MASS)
...
# f(x;a) = c*(x-a)*(a+20-x) on [a, a+20], c = 3/4000 (see 1a)
f_wind <- function(x, a) {
  ifelse(x >= a & x <= a + 20, (3 / 4000) * (x - a) * (a + 20 - x), 0)
}

# cdf F(x;a) on [a, a+20] (see 1c)
F_wind <- function(x, a) {
  u <- pmin(pmax(x - a, 0), 20)
  (3 / 4000) * (10 * u^2 - u^3 / 3)
}

# 1d - invert F numerically with uniroot, no clean closed form for F^-1
# (handle p=0/1 separately since uniroot needs opposite-signed endpoints)
...
# 1f - acceptance-rejection with a uniform proposal g(x) = 1/20 on [a,a+20]
M <- 1.5  # M = 20*max f(x;a) = 20*0.075 = 1.5, see 1e(i)
...
# 2d(i) MLE of beta is just xbar (see 2a)
beta_hat <- mean(rain)

# 2d(ii) approx 95% CI via CLT: beta_hat +/- 1.96*beta_hat/sqrt(n)
...
# 3d - fit a gamma via MASS::fitdistr and compare against KDE bandwidths
fit_gamma <- tryCatch(
  fitdistr(rain, "gamma"),
  error = function(e) {
    stop("fitdistr() failed on rain: ", conditionMessage(e))
  }
)
...
bw_small <- bw_default / 5   # too small
bw_large <- bw_default * 4   # too large
...
message("done - see output/console_log.txt and output/mandatory1_plots.pdf")
